    ...
    for petID in rematch.roster:AllPets() do
        ...
        if petInfo.isOwned then
            ...
            if type(petInfo.rarity)=="number" and petInfo.rarity>0 then
                -- [Community fix] off-by-one: was reading info[5+rarity] but writing info[6+rarity]
                info[6+petInfo.rarity] = info[6+petInfo.rarity]+1 -- rarity takes up 7th through 10th indexes
            end
