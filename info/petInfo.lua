...
-- [Community fix] the chained :match():gsub() crashed if either Blizz API returned nil or
-- the description text didn't contain a \r\n…\r block (localization/format changes).
function funcs:Passive()
    local petType = self.petType
    if petType and not self.isSpecialType then
        local _,_,_,_,desc = C_PetBattles.GetAbilityInfoByID(PET_BATTLE_PET_TYPE_PASSIVES[petType])
        local extracted = type(desc)=="string" and desc:match("^.-\r\n(.-)\r") or nil
        self.passive = extracted and extracted:gsub("%[percentage.-%]%%","4%%") or ""
    end
end
