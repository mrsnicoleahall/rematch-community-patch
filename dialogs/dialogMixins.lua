...
function RematchDialogGroupPickerMixin:Update()
    ...
    -- [Community feature] when invoked from the Save Team flow, append a synthetic
    -- "+ Create New Group..." entry so the user can create a group inline.
    if rematch.dialog.GetOpenDialog and rematch.dialog:GetOpenDialog()=="SaveTeam" then
        tinsert(self.groupList,"__newgroup__")
    end
    self.List:Update()
end
...
function RematchDialogGroupPickerMixin:FillGroup(groupID)
    ...
    -- [Community feature] render the synthetic "+ Create New Group..." entry differently
    if groupID=="__newgroup__" then
        self.Text:SetText("|cff7ec2ff+ "..(L["Create New Group"] or "Create New Group").."...|r")
        self.Icon:SetTexture("Interface\\PaperDollInfoFrame\\Character-Plus")
        if self.Badges and self.Badges.Hide then self.Badges:Hide() end
        self.Text:SetPoint("BOTTOMRIGHT",-22,2)
        return
    end
...
function RematchDialogGroupPickerListButtonMixin:OnClick(button)
    if button~="RightButton" then
        ...
        -- [Community feature] handle the synthetic "+ Create New Group..." entry
        if self.groupID=="__newgroup__" then
            if rematch.saveDialog and rematch.saveDialog.InlineCreateGroup then
                rematch.saveDialog:InlineCreateGroup(picker.returnLayout or "Default")
            end
            return
        end
