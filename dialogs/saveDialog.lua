...
function rematch.saveDialog:Accept(info,subject)
    -- [Community fix] defensive name coercion to avoid :lower() on nil during edge cases
    local sidelineName = type(rematch.savedTeams.sideline.name)=="string" and rematch.savedTeams.sideline.name or ""
    local originalName = type(rematch.savedTeams.original.name)=="string" and rematch.savedTeams.original.name or ""
    if rematch.saveDialog:IsTeamNameUsed() then -- if the team's name will overwrite another, bring up overwrite dialog
        local overwriteID = rematch.savedTeams:GetTeamIDByName(sidelineName)
    ...
    elseif rematch.savedTeams:IsUserTeam(subject.teamID) and sidelineName:lower()==originalName:lower() then
...
-- [Community fix] coerce names safely; original could be unset on edge cases
function rematch.saveDialog:IsTeamNameUsed()
    local sidelineName = type(rematch.savedTeams.sideline.name)=="string" and rematch.savedTeams.sideline.name or ""
    local originalName = type(rematch.savedTeams.original.name)=="string" and rematch.savedTeams.original.name or ""
    if sidelineName:lower()~=originalName:lower() and rematch.savedTeams:GetTeamIDByName(sidelineName) then
        return true
    else
        return false
    end
...
function rematch.saveDialog:Refresh(info,subject,firstRun)
    ...
    if layout=="Default" then
        ...
        -- [Community fix] guard against nil group lookup (sideline.groupID may not match a saved group mid-edit)
        local _sidelineGroup = rematch.savedGroups[rematch.savedTeams.sideline.groupID]
        self.ComboBox:SetTextColor(rematch.utils:HexToRGB(settings.ColorTeamNames and _sidelineGroup and _sidelineGroup.color or "E8E8E8"))
...
function rematch.saveDialog:OnChange(info,subject)
    ...
    -- [Community fix] ComboBox can return nil mid-layout-swap
    local comboText = self.ComboBox:GetText() or ""
    sideline.name = comboText:trim()
...
--[[ [Community feature] Inline group creation from the Save Team dialog ]]

-- Stash for the in-progress save when we detour into EditGroup.
-- Lives at module scope so the EditGroup wrapper can find it after dialog round-trip.
rematch.saveDialog._inlineGroupReturn = nil

-- Called from the GroupPicker's synthetic "+ Create New Group..." entry.
-- Stashes the current SaveTeam subject so we can return to it, then opens EditGroup.
function rematch.saveDialog:InlineCreateGroup(returnLayout)
    -- Capture the current SaveTeam dialog state so we can re-open it after EditGroup.
    local info = rematch.dialog:GetDialogInfo("SaveTeam")
    local subject = info and info.subject
    if not subject then return end
    rematch.saveDialog._inlineGroupReturn = {
        saveMode = subject.saveMode,
        teamID = subject.teamID,
        returnLayout = returnLayout or "Default",
        -- pre-save data we'd otherwise lose during the dialog swap
        sidelineGroupID = rematch.savedTeams.sideline.groupID,
    }
    -- Hand off to the existing EditGroup dialog (subject=nil means "new group")
    rematch.dialog:ShowDialog("EditGroup", nil)
end

-- Internal: called by the EditGroup acceptFunc tail. Re-opens SaveTeam with the
-- newly-created group selected on the sideline. If groupID is nil (creation
-- failed somehow), we still come back to the SaveTeam dialog with the prior group.
function rematch.saveDialog:_FinishInlineCreateGroup(newGroupID)
    local ret = rematch.saveDialog._inlineGroupReturn
    rematch.saveDialog._inlineGroupReturn = nil
    if not ret then return end
    if newGroupID and rematch.savedGroups[newGroupID] then
        rematch.savedTeams.sideline.groupID = newGroupID
        settings.LastSelectedGroup = newGroupID
    else
        -- Restore the previous group on the sideline if the creation didn't yield one we can find
        rematch.savedTeams.sideline.groupID = ret.sidelineGroupID or rematch.savedTeams.sideline.groupID
    end
    -- Re-open the Save Team dialog. fromOverwrite=true keeps the existing "original"
    -- baseline intact so the Reset button still does the right thing.
    rematch.dialog:ShowDialog("SaveTeam",{
        saveMode = ret.saveMode,
        teamID = ret.teamID,
        fromOverwrite = true,
    })
end
