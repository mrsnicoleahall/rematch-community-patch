...
-- [Community fix] Do NOT mutate source.name in place. Trim into a local and write to dest only.
local function copyTeam(source,dest)
    assert(type(dest)=="table" and dest.teamID,"Attempt to copy to a malformed team: "..((type(source)=="table" and source.teamID) or "nil").." to "..(type(dest)=="table" and "nil" or "non-table"))
    assert(validateTeamStructure(source),"Attempt to copy from a malformed team: "..((type(source)=="table" and source.teamID) or "nil").." to "..(dest.teamID or "nil"))
    local trimmedName = (type(source.name)=="string" and source.name or ""):trim()
    ...
    for k,v in pairs(source) do
        ...
        elseif k=="name" then
            dest.name = trimmedName -- always write the trimmed name, never overwrite from source raw
    ...
    -- guarantee a name (even if source had nil somehow)
    if type(dest.name)~="string" then
...
local function setter(self,key,value)
    ...
    elseif validateTeamStructure(value) then -- if assigning another team to this team
        -- [Community fix] guard nil name; copyTeam now handles the trim safely
...
-- [Community fix] guard against nil/non-string names from corrupt savedvars
local function updateNames()
    wipe(teamIDsByName)
    for teamID,team in rematch.savedTeams:AllTeams() do
        local name = type(team.name)=="string" and team.name or ""
        teamIDsByName[name:lower()] = teamID
    end
end
...
-- [Community fix] setter already dispatches REMATCH_TEAM_DELETED and calls TeamsChanged.
-- Calling TeamsChanged again here caused listeners to fire twice, with the second pass
-- sometimes hitting stale group.teams entries referring to the just-deleted teamID.
...
-- [Community fix] original only handled moves in/out of favorites and silently no-op'd
-- on regular group-to-group moves. Now handles all four cases.
function rematch.savedTeams:MoveTeam(teamID,groupID)
    if not (rematch.savedTeams:IsUserTeam(teamID) and groupID and rematch.savedGroups[groupID]) then
        return false
    end
    local team = rematch.savedTeams[teamID]
    if team.groupID == groupID then
        return false -- already in this group
    end
...
-- [Community fix] coerce non-string input safely
function rematch.savedTeams:GetUniqueName(name)
    updateNames()
    if type(name)~="string" then name = L["New Team"] end
    name = name:trim()
    if name=="" then name = L["New Team"] end
    if not teamIDsByName[name:lower()] then
        return name -- name given (or "New Team") is not taken yet, return it
    end
    name = name:gsub(" %(%d+%)$","") -- take off any trailing (2)s
...
-- [Community fix] tolerate non-string input rather than throwing on :lower()
function rematch.savedTeams:GetTeamIDByName(name)
    return type(name)=="string" and teamIDsByName[name:lower()] or nil
end
