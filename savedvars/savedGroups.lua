...
local function teamWinSort(e1,e2)
    ...
    elseif team1.winrecord and team2.winrecord then
        ...
        if settings.AlternateWinRecord then -- sorting by raw wins instead of a calculation
            ...
            -- [Community fix] guard against divide-by-zero when battles is missing or 0
            local battles1 = team1.winrecord.battles or 0
            local battles2 = team2.winrecord.battles or 0
            if battles1>0 and battles2>0 then
                local percent1 = wins1/battles1
                local percent2 = wins2/battles2
                if percent1~=percent2 then
                    return percent1>percent2
                end
            elseif battles1>0 then
                return true
            elseif battles2>0 then
                return false
            end
        end
    end
    -- if we reached here, teams are the same, sort by name first and then teamID last
    -- [Community fix] coerce names so a corrupt savedvar can't crash the sort
    local name1 = type(team1.name)=="string" and team1.name or ""
    local name2 = type(team2.name)=="string" and team2.name or ""
    if name1~=name2 then
        return name1<name2
    else
        return e1<e2
