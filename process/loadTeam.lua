...
function startLoad(teamID)
    ...
    if team then
        ...
        -- [Community fix] original referenced undefined `slot` (loop var is `i`) so this check
        -- never detected non-random pets and silently misbehaved.
        ...
        for slot=1,3 do
            ...
            if petID then
                ...
                elseif petInfo.idType=="pet" and not petInfo.isValid then
                    -- [Community fix] ability1/2/3 were undefined in this branch; pull them from team.tags
                    local ability1,ability2,ability3 = rematch.petTags:GetAbilities(team.tags[slot])
