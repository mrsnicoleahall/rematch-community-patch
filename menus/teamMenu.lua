...
rematch.events:Register(rematch.teamMenu,"PLAYER_LOGIN",function(self)
    ...
    rematch.dialog:Register("EditGroup",{
        ...
        acceptFunc = function(self,info,subject)
            local group = subject and rematch.savedGroups[subject]
            -- [Community fix] :trim() on empty returns "" which is truthy, so the original
            -- `or L["New Group"]` never kicked in. Use a length check.
            local typed = self.EditBox:GetText() or ""
            typed = typed:trim()
            local name = (typed~="" and typed) or L["New Group"]
            if not group then
            ...
            -- [Community feature] if EditGroup was opened from the inline "+ Create New Group..."
            -- entry in the Save Team dialog, chain back to SaveTeam with the new group selected.
            if rematch.saveDialog and rematch.saveDialog._inlineGroupReturn then
                local newGroupID = group and group.groupID
                C_Timer.After(0,function()
                    rematch.saveDialog:_FinishInlineCreateGroup(newGroupID)
                end)
            end
        end,
        -- [Community feature] cancelling EditGroup when it was opened inline should
        -- still bring the user back to the Save Team dialog (with the prior group selected).
        cancelFunc = function(self,info,subject)
            if rematch.saveDialog and rematch.saveDialog._inlineGroupReturn then
                C_Timer.After(0,function()
                    rematch.saveDialog:_FinishInlineCreateGroup(nil)
                end)
            end
        end,
    })
...
function rematch.teamMenu:SetOrRemoveFavorite(teamID)
    local team = teamID and rematch.savedTeams[teamID]
    if not team then return end -- [Community fix] guard before else branch
...
function rematch.teamMenu:ExportTeam(teamID)
    if rematch.menus:IsMenuOpen("LoadedTeamMenu") then -- if this is the export from the loadedTeamPanel Share -> Export
        -- [Community fix] previously passed an undefined upvalue `newTeam` (always nil); pass nil explicitly
        rematch.saveDialog:SidelineLoadouts(nil) -- then export pets actually loaded
...
-- [Community fix] guard against nil targets table before doing #targets
