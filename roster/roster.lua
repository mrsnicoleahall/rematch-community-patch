...
-- [Community fix] Defensive resolution of the pet journal collected/not-collected filter constants.
-- In some WoW client builds Enum.PetJournalFilter is nil, in others the LE_PET_JOURNAL_FILTER_*
-- globals are nil. We probe both and fall back to the documented integer values (1 and 2).
-- Source of nil-Enum crash captured via BugGrabber:
--   "Interface/AddOns/Rematch_Community/roster/roster.lua:228: attempt to index field 'PetJournalFilter' (a nil value)"
local FILTER_COLLECTED =
    (Enum and Enum.PetJournalFilter and Enum.PetJournalFilter.Collected)
    or LE_PET_JOURNAL_FILTER_COLLECTED
    or 1
local FILTER_NOT_COLLECTED =
    (Enum and Enum.PetJournalFilter and Enum.PetJournalFilter.NotCollected)
    or LE_PET_JOURNAL_FILTER_NOT_COLLECTED
    or 2
...
function rematch.roster:IsAnyFilterUsed()
    ...
    if not C_PetJournal.IsFilterChecked(FILTER_COLLECTED) then
        return true -- collected is unchecked
    end
    if not C_PetJournal.IsFilterChecked(FILTER_NOT_COLLECTED) then
