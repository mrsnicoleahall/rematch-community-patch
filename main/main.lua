...
rematch.events:Register(rematch.main,"PLAYER_LOGIN",function(self)
    ...
    -- [Community fix] warn the user if both the original Rematch and this fork are enabled,
    -- since they share global frames (RematchFrame, RematchDialog, etc.) and will conflict.
    if C_AddOns and C_AddOns.IsAddOnLoaded and C_AddOns.IsAddOnLoaded("Rematch") then
        local msg = "|cff7ec2ffRematch_Community:|r The original |cffffff00Rematch|r addon is also enabled. "
            .."Please disable the original Rematch in the AddOns menu and reload your UI to avoid conflicts."
        if DEFAULT_CHAT_FRAME and DEFAULT_CHAT_FRAME.AddMessage then
            DEFAULT_CHAT_FRAME:AddMessage(msg)
        end
        if UIErrorsFrame and UIErrorsFrame.AddMessage then
            UIErrorsFrame:AddMessage("Rematch_Community: Disable original Rematch addon",1,0.3,0.3,1)
        end
    end
